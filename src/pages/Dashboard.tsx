...
export default function Dashboard() {
  ...
  // ─── Operational Pulse (Last 15 Days) ───
  const pulseData = useMemo(() => {
    ...
    return dates.map(date => {
      const vCount = visits.filter(v => v.fecha === date).length;
      const tCount = tasks.filter(t => t.createdAt.split('T')[0] === date).length;
      const nCount = novedades.filter(n => (n.fecha || n.createdAt).split('T')[0] === date).length;
  ...
  const heatmapPoints = useMemo(() => {
    ...
    visits.forEach(v => {
      const loc = locationMap.get(v.destino);
      if (loc?.latitude && loc?.longitude) {
        points.push([loc.latitude, loc.longitude, 1]);
      }
    });
    return points;
  }, [visits, locations]);
  ...
  if (loading) {
    return (
      <div className="font-['Inter'] max-w-7xl mx-auto p-6">
        <div className="h-10 w-48 bg-[#1a1a1a]/10 animate-pulse mb-8 border-2 border-[#1a1a1a]"></div>
    ...
    <div className="font-['Inter'] max-w-7xl mx-auto px-3 sm:px-6 pb-24 lg:pb-12">
      ...
      <AnimatePresence>
        {overdueCount > 0 && (
          ...
          >
            <div 
              className="p-4 bg-[#e63b2e] text-white border-2 border-[#1a1a1a] shadow-[6px_6px_0px_0px_rgba(26,26,26,1)] flex items-center justify-between cursor-pointer hover:bg-[#1a1a1a] transition-all group"
              onClick={() => navigate('/tareas')}
            >
              <div className="flex items-center gap-4">
                <div className="bg-white text-[#e63b2e] p-2 border-2 border-[#1a1a1a]">
                  <AlertTriangle className="w-6 h-6 animate-bounce" />
                </div>
                <div>
                  <p className="font-black uppercase text-lg tracking-tight">ALERTA: {overdueCount} TAREAS VENCIDAS</p>
                  <p className="text-[10px] font-bold opacity-80 uppercase tracking-widest">REQUIEREN ATENCIÓN INMEDIATA POR PARTE DEL PERSONAL ASIGNADO</p>
                </div>
              </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-1 gap-4">
            {summaryCards.map((card, idx) => (
              <motion.div
                ...
                className="p-4 border-2 border-[#1a1a1a] bg-white shadow-[4px_4px_0px_0px_rgba(26,26,26,1)] flex items-center gap-4 cursor-pointer"
              >
                <div className={`p-3 border-2 border-[#1a1a1a] ${card.color} text-white`}>
                  <card.icon className="w-5 h-5" />
                </div>
                <div>
                  <p className="text-[9px] font-black uppercase opacity-40 tracking-tighter leading-none mb-1">{card.label}</p>
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="border-2 border-[#1a1a1a] bg-white p-6 shadow-[6px_6px_0px_0px_rgba(26,26,26,1)]">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-4">
                <div className="flex items-center gap-1.5"><div className="w-2 h-2 bg-[#00cc66]"></div><span className="text-[9px] font-black">VISITAS</span></div>
                <div className="flex items-center gap-1.5"><div className="w-2 h-2 bg-[#0055ff]"></div><span className="text-[9px] font-black">TAREAS</span></div>
                <div className="flex items-center gap-1.5"><div className="w-2 h-2 bg-[#1a1a1a]"></div><span className="text-[9px] font-black">NOVEDADES</span></div>
              </div>
            </div>
          ...
          <div className="border-2 border-[#1a1a1a] bg-[#f5f0e8] p-6 shadow-[6px_6px_0px_0px_rgba(26,26,26,1)]">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-black uppercase tracking-widest flex items-center gap-2 text-[#1a1a1a]">
                <MapIcon className="w-4 h-4" /> DENSIDAD OPERATIVA (MAPA DE CALOR)
              </h3>
              <span className="text-[10px] font-black bg-[#1a1a1a] text-white px-2 py-0.5 border-2 border-[#1a1a1a]">
                {heatmapPoints.length} PUNTOS DE ACTIVIDAD
              </span>
            </div>
            <div className="h-[400px] border-2 border-[#1a1a1a] relative overflow-hidden grayscale-[0.5] contrast-[1.2]">
              ...
              <div className="absolute bottom-4 right-4 bg-white border-2 border-[#1a1a1a] p-3 shadow-[3px_3px_0px_0px_rgba(26,26,26,1)] z-[1000]">
                 <p className="text-[8px] font-black uppercase tracking-widest opacity-50 mb-2">INTENSIDAD DE PATRULLAJE</p>
                 <div className="h-2 w-full bg-gradient-to-r from-[#0055ff] via-[#00cc66] to-[#e63b2e] border border-[#1a1a1a]"></div>
                 <div className="flex justify-between mt-1">
                   <span className="text-[8px] font-black uppercase">BAJA</span>
