...
interface TimelineItem {
  ...
  title: string;
  created_at: string; // Updated to match database column
  description?: string;
  author_name?: string;
}
  ...
  return (
    <div className="relative border-l-2 border-slate-200 ml-3 space-y-6">
      {items.map((item) => {
        ...
        const dateFormatted = isNaN(dateObj.getTime()) ? 'Fecha inválida' : dateObj.toLocaleDateString();
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="inline-block px-2 py-0.5 text-[10px] font-bold uppercase rounded bg-slate-100 text-slate-600 shrink-0">
                      {item.type}
                    </span>
                ...
                {expandedId === item.id ? <ChevronUp className="w-5 h-5 text-slate-400 shrink-0 ml-2" /> : <ChevronDown className="w-5 h-5 text-slate-400 shrink-0 ml-2" />}
              </button>
              
              {expandedId === item.id && (
                <div className="px-4 pb-4 border-t border-slate-100 pt-3 text-sm text-slate-600 break-words">
                  <p className="whitespace-pre-wrap">{item.description || 'Sin detalles adicionales disponibles.'}</p>
